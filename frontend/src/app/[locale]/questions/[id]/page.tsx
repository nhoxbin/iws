...
export default function QuestionDetailPage() {
  ...
  const handleMarkHelpful = async (answerId: number) => {
    try {
      // You'll need to create the backend endpoint for this
      await api.post(`/answers/${answerId}/helpful`);
      ...
      setAnswers(answers.map(answer =>
        answer.id === answerId
          ? {
              ...
              helpful_count: (answer.helpful_count || 0) + (answer.is_helpful ? -1 : 1)
            }
          : answer
      ));
    } catch (err) {
            ...
            <div className="flex items-start justify-between gap-4 mb-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="p-2 text-slate-600 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition"
                  title={isSaved ? 'Unsave question' : 'Save question'}
                >
                  {isSaved ? <BookmarkCheck className="w-5 h-5" /> : <Bookmark className="w-5 h-5" />}
                </button>
                {currentUser && currentUser.id === question.user.id && (
                  <>
                    <Link
                    ...
                    <button
                      ...
                      className="p-2 text-slate-600 dark:text-slate-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition"
                    >
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-slate-200 dark:border-slate-800">
                      ...
                      >
                        ...
                        <span className="text-sm font-medium">
                          Helpful {answer.helpful_count ? `(${answer.helpful_count})` : ''}
                        </span>
                      </button>
                    </div>
                  </article>
